...
public class ControllerRepuesto {
    ...
    @GetMapping("/repuesto/precio/{precio}")
    public List<ResponseRepuesto> consultarRepuestoPrecioMenorA(@PathVariable(name = "precio") double precio) {
         return serviceRepuesto.consultarPorPrecioMenorA(precio).stream().map(r->mapperRepuesto.toResponse(r)).toList();
    }
    
    @DeleteMapping("/repuesto/{id}")
    public void eliminarRepuesto(@PathVariable(name = "id") Integer id) {
        serviceRepuesto.eliminarRepuesto(id);
    }
}
